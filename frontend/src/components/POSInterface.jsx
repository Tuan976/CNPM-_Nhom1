...
export default function POSInterface() {
  ...
  return (
    <div className="grid gap-5 h-[calc(100vh-200px)]" style={{ gridTemplateColumns: '1fr 340px' }}>

      {/* ── LEFT: Products ── */}
      <div className="flex flex-col gap-4 overflow-hidden">
        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2" size={18} style={{ color: 'var(--bc-ink-300)' }} />
          <input value={q} onChange={e => { setQ(e.target.value); fetchProducts(e.target.value); }}
        ...
        <div className="grid gap-4 overflow-y-auto custom-scrollbar pb-4"
      ...
      <div className="flex flex-col rounded-3xl overflow-hidden bg-white"
        ...
        <div className="p-5 space-y-4" style={{ borderTop: '2px solid var(--bc-ink-100)', background: 'var(--bc-bg-soft)' }}>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between text-[12px]" style={fb}>
              <span style={{ color: 'var(--bc-ink-400)' }}>Giảm giá</span>
              <span style={{ color: 'var(--bc-teal)' }}>0đ</span>
            </div>
          </div>
          ...
          <button onClick={() => setShowCheckout(true)}
            ...
            <Zap size={18} /> Tiến hành thanh toán
          </button>
        </div>
      </div>
